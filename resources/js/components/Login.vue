<template>
  <div className="tw-h-screen tw-w-full tw-flex tw-items-center tw-justify-center">
    <div className="tw-fixed tw-inset-0">
      <img
        src="/storage/static/images/academia.jpeg"
        className="tw-w-full tw-h-full tw-object-cover"
        alt=""
      />
    </div>
    <div
      className="tw-fixed tw-inset-0 tw-flex tw-items-center tw-justify-center tw-backdrop-blur-md tw-bg-blue-900/30"
    >
      <div
        className="tw-z-10 tw-bg-white tw-border tw-shadow-lg tw-rounded-lg tw-p-6 tw-max-w-md"
      >
  ...
  methods: {
    async login() {
      try {
        ...
        // Verifica se a resposta do servidor contém o token
        if (response.data.token) {
          // Armazene o token em algum lugar seguro, como LocalStorage ou Vuex
